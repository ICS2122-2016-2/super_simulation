...
class Mapa:
    ...
    def __init__(self, X, Y, familias, sobrantes, periodo):
    ...
    def devolver_familias(self):
        # with open('Datos/calvesProductosP{}'.format(self.periodo), 'rb') as file:
        #         claves = pickle.load(file)
        #
        # for i in range(len(self.familias)):
        #     for j in range(len(self.familias[i])):
        #         self.familias[i][j] = claves[self.familias[i][j]]
        # for i in range(len(self.sobrantes)):
        #     for j in range(len(self.sobrantes[i])):
        #         self.sobrantes[i][j] = claves[self.sobrantes[i][j]]
        return [self.familias, self.sobrantes]
...
def construir_mapas(archivo_familias, archivo_sobrantes, optimizacion=0, periodo=0):
    if not optimizacion:
        with open(archivo_familias, 'r') as file:
            familias = json.load(file)
        with open(archivo_sobrantes, 'r') as file:
            sobrantes = json.load(file)
    else:
